...
jsi::Value JsiHostObject::get(jsi::Runtime& runtime, const jsi::PropNameID& name) {
  ...
  auto hostFunctionCache = _hostFunctionCache.find(static_cast<void*>(&runtime));
  ...
  if (func != funcs.end()) {
    ...
    return hostFunctionCache->second.emplace(nameStr, jsi::Function::createFromHostFunction(runtime, name, 0, dispatcher))
